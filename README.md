...
点击“读取并编辑 OCR 文本”会按当前页码范围和每段页数读取 OCR，并将每段文字填入文本框；
用户可以直接纠正错字、删改内容，修改后的文本会用于 TTS 伴读。
如果随后直接点击“开始生成影片”，程序会复用这次 OCR 结果，不再重复识别；修改后的文字
只覆盖伴读内容，原始 OCR 结果仍保留用于 AI 剧情分析。
读取前请先设置页面范围、每段页数和 OCR 语言；读取过程中按钮会变为“取消 OCR 读取”，
可中止浏览器等待并继续修改配置。
...
启用 TTS 时，生成配音前会暂停显示每个场景的伴读文本。每行对应一个场景，可编辑后确认；
如果 60 秒内没有操作，系统会自动使用当前文本继续生成配音。
