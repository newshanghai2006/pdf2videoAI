...
| **LLM RPM 限速** | 文本 LLM 每分钟最多请求数，默认 20，可在 1-600 之间修改；相同服务地址、Key 和模型的并发任务共享限制 |
...
| **合成前审核图片和解说词** | 默认开启。画面准备完成后逐场景显示图片和解说词；可以修改文字，或上传 PNG/JPG/WEBP 替换 AI 图片，再进入 TTS 和影片合成 |
...
启用 AI 剧情分析后，程序会为主要人物建立统一视觉档案，包括年龄、国籍、脸型五官、发型、体型、服装剪裁与固定色彩、徽记、配饰和道具。每个场景、AI 封面、AI 图片和 Agnes 视频提示词都会复用相同的人物身份锚点；长 PDF 分批分析时，后一批也会继承前面已经确认的人物档案。该方法能明显减少换脸、服装变色和人物身份漂移，但最终一致性仍受所用图像/视频模型能力影响。
...
NVIDIA 图像请求仍按免费额度 40 RPM 在进程内限速。NVIDIA 文本 LLM 使用界面中的
“LLM RPM 限速”（默认 20）；连接测试、剧情分析和重试均遵守该数值。相同服务地址、Key
和模型的多个任务共享文本限速器；服务重启后限速状态重新计时。
...
| 能力 | 免费实际限制 | 程序处理 |
|------|-------------|----------|
| `agnes-2.0-flash` 文本 | 20 RPM | 界面默认填 20，可按账户实际额度修改；剧情分析、连接测试和重试共享用户设定 |
...
常用 SenseNova 文本模型已经加入 LLM 模型下拉框。界面的“LLM RPM 限速”默认 20；使用
SenseNova 时应把公开的 5 小时额度换算成保守的每分钟数，例如：

| Model ID | 公开额度 | 建议填写 RPM |
|----------|----------|------------------|
| `sensenova-6.7-flash-lite` | 每 5 小时 1500 次 | 5 RPM |
| `sensenova-u1-fast` | 每 5 小时 1500 次 | 5 RPM |
| `deepseek-v4-flash`（SenseNova） | 每 5 小时 500 次 | 1 RPM（保守） |

连接测试、剧情分析批次、自动重试和用户点击重试共享同一个进程内限速器。多个 Flask 进程
或其他程序使用同一 Key 时无法互相协调，服务端额度仍是最终依据。用户设定只控制文本 LLM；
图片和视频仍分别遵守各自模型的 RPM 限制。
...
│   ├── rate_limiter.py         # 用户可配置的 LLM RPM 及 NVIDIA / Agnes 专用限速器
...
OCR 不在配置页单独预览。开始生成后只执行一次 OCR，随后在 TTS/影片合成前的场景审核界面中显示
...
“我的任务”弹窗顶部可直接进入“新任务”。点击后会关闭弹窗、清理当前页面状态并返回 PDF 上传页；即使旧版缓存页面缺少某个可选控件，也不会阻断新任务入口。已完成、失败或已暂停且没有运行线程的任务可点击
...
默认在 TTS 和影片合成前暂停显示场景卡片；每个场景图片下方都有独立的伴读文本框和
“上传替换图片”按钮。上传的 PNG/JPG/WEBP 会转换为标准 RGB PNG，随后用于 TTS 对应场景、
ComfyUI 导出和最终视频。开始编辑或选择图片后倒计时停止；未操作时 60 秒自动采用当前内容继续。
审核阶段后端保留 3600 秒兜底等待：即使浏览器进入后台、网络中断或 JavaScript 定时器被
系统节流，超时后也会采用当前已保存的图片和文字继续，不会把任务停在确认阶段。关闭
“合成前审核图片和解说词”后，若启用 TTS，则继续使用原有的纯文字确认和 180 秒兜底流程。
...
用户点击上传按钮或开始编辑任意场景文本后，倒计时会停止，需要手动点击确认。此时页面
显示的是“审核图片和解说词”，不是错误状态；下一次状态轮询不会重新启动倒计时。
...
| POST | `/api/tasks/<task_id>/scenes/<scene_idx>/image` | 审核阶段上传并替换单个场景图片 |
