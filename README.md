...
降低音频拼接时的封装失败概率。对于超过 16 个场景的长 PDF，程序会自动分批拼接视频和
音轨，再合并批次，不会在一条 FFmpeg 命令中同时打开全部场景文件。这可避免服务器因文件
句柄、内存或滤镜图资源限制而在日志显示大量 `Stream #... -> setpts` 后失败；该处理不会
改变场景顺序、设定时长或无 TTS 时生成的静音轨。
